...
int main(int argc, char* argv[]) {
	...
	for (int i = 0; i < numLumps; ++i) {
		...
		if (lump->compressedSize != lump->size)
		{
			...
			int res = Kraken_Decompress(lump->data, lump->compressedSize, dst, lump->size);
			if (res < 0) {
				printf("Decompression failed with error code %d\n", res);
				return 1;
			}
			if (res != lump->size) {
				printf("Decompressed size mismatch: expected %llu, got %d\n", lump->size, res);
				return 1;
			}
			...
			fclose(f);
			free(dst);
			printf("Wrote out compressed lump %s, uncompressed size: %llu\n", lump->name, lump->size);
		}
		else
		{
			FILE* f = fopen(lump->name, "wb");
	...
	snprintf(path, sizeof(path), "%s/base/sound/soundbanks/pc/sound.pack", argv[1]);
	if (LoadWolf2Lumps(path, &lumps, &numLumps) != 0) {
		return 1; // Error loading lumps
	}
	...
	// patch_1_english(us).pack contains sb_vo_wolfstone.bnk (need to read english(us).pack and patch_\d_english(us).pack)
	for (int i = 0; ; i++)
	{
		if (i == 0)
		{
			snprintf(path, sizeof(path), "%s/base/sound/soundbanks/pc/english(us).pack", argv[1]);
		}
		else
		{

			snprintf(path, sizeof(path), "%s/base/sound/soundbanks/pc/patch_%d_english(us).pack", argv[1], i);
		}
		if (LoadWolf2Lumps(path, &lumps, &numLumps) != 0) {
			break;
		}
