...
typedef hashmap_uint32_t (*hashmap_hasher_t)(
	hashmap_uint32_t seed, const void *key, hashmap_uint32_t key_len);
typedef int (*hashmap_comparer_t)(
	const void *a, hashmap_uint32_t a_len, const void *b,
...
extern "C"
{
	...
	HASHMAP_WEAK int hashmap_create(
	...
	HASHMAP_WEAK int hashmap_create_ex(
	...
	/// @return On success 0 is returned.
	///
	/// The key string slice is not copied when creating the hashmap entry, and
	/// thus must remain a valid pointer until the hashmap entry is removed or
	/// the hashmap is destroyed.
	HASHMAP_WEAK int hashmap_put(
		struct hashmap_s *const hashmap, const void *const key,
	...
	HASHMAP_WEAK void *hashmap_get(
		const struct hashmap_s *const hashmap, const void *const key,
	...
	/// @param len The length of the string key.
	/// @return On success 0 is returned.
	HASHMAP_WEAK int hashmap_remove(
		struct hashmap_s *const hashmap, const void *const key,
		const hashmap_uint32_t len);
	...
	/// @param key The string key to use.
	/// @param len The length of the string key.
	/// @return On success the original stored key pointer is returned, on
	/// failure NULL is returned.
	HASHMAP_WEAK const void *hashmap_remove_and_return_key(
		struct hashmap_s *const hashmap, const void *const key,
		const hashmap_uint32_t len);
	...
	/// Otherwise if the callback function f returned non-zero then non-zero is
	/// returned.
	HASHMAP_WEAK int hashmap_iterate(
		const struct hashmap_s *const hashmap,
		int (*iterator)(void *const context, void *const value),
		void *const context);
	...
	/// @param context The context to pass as the first argument to f.
	/// @return If the entire hashmap was iterated then 0 is returned.
	/// Otherwise if the callback function f returned positive then the
	/// positive value is returned.  If the callback function returns -1, the
	/// current item is removed and iteration continues.
	...
	static hashmap_uint32_t hashmap_crc32_hasher(
		const hashmap_uint32_t seed, const void *const s,
		const hashmap_uint32_t len);
	static int hashmap_memcmp_comparer(
		const void *const a, const hashmap_uint32_t a_len, const void *const b,
	...
	HASHMAP_ALWAYS_INLINE hashmap_uint32_t hashmap_hash_helper_int_helper(
		...
		const hashmap_uint32_t len);
	HASHMAP_ALWAYS_INLINE int hashmap_hash_helper(
		const struct hashmap_s *const m, const void *const key,
		const hashmap_uint32_t len, hashmap_uint32_t *const out_index);
	HASHMAP_WEAK int hashmap_rehash_iterator(
		void *const new_hash, struct hashmap_element_s *const e);
	...
	struct hashmap_s *const out_hashmap)
{
	...
	else if (0 != (options.initial_capacity & (options.initial_capacity - 1)))
	{
		options.initial_capacity =
			1u << (32 - hashmap_clz(options.initial_capacity));
	}
	...
	out_hashmap->data = HASHMAP_CAST(
		...
		calloc(
			options.initial_capacity + HASHMAP_LINEAR_PROBE_LENGTH,
...
int hashmap_put(
	struct hashmap_s *const m, const void *const key,
	const hashmap_uint32_t len, void *const value)
{
	...
	const hashmap_uint32_t len)
{
	...
	for (i = 0; i < HASHMAP_LINEAR_PROBE_LENGTH; i++)
	{
		...
		{
			if (m->comparer(
					m->data[index].key, m->data[index].key_len, key, len))
			{
...
const void *hashmap_remove_and_return_key(
	struct hashmap_s *const m, const void *const key,
	const hashmap_uint32_t len)
{
	hashmap_uint32_t i, curr;
...
int hashmap_iterate(
	const struct hashmap_s *const m, int (*f)(void *const, void *const),
	void *const context)
{
	hashmap_uint32_t i;
...
hashmap_uint32_t hashmap_crc32_hasher(
	const hashmap_uint32_t seed, const void *const k,
	const hashmap_uint32_t len)
{
	hashmap_uint32_t i = 0;
	hashmap_uint32_t crc32val = seed;
	const hashmap_uint8_t *const s =
		HASHMAP_PTR_CAST(const hashmap_uint8_t *, k);
	...
	{
		crc32val =
			crc32_tab[(HASHMAP_CAST(hashmap_uint8_t, crc32val) ^ s[i])] ^
			(crc32val >> 8);
	}
#endif
...
HASHMAP_ALWAYS_INLINE hashmap_uint32_t hashmap_hash_helper_int_helper(
	const struct hashmap_s *const m, const void *const k,
...
int hashmap_rehash_iterator(
	void *const new_hash, struct hashmap_element_s *const e)
{
	int temp = hashmap_put(
		HASHMAP_PTR_CAST(struct hashmap_s *, new_hash), e->key, e->key_len,
		e->data);
...
HASHMAP_ALWAYS_INLINE int hashmap_rehash_helper(struct hashmap_s *const m)
{
	...
	flag = hashmap_iterate_pairs(
		m, hashmap_rehash_iterator, HASHMAP_PTR_CAST(void *, &new_m));
